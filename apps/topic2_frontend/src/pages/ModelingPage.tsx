/** 工艺建模：E2P Model Policy → 多模型训练比较 → 系统推荐 / 人工覆盖。
 *  人工覆盖以 Level-2 Proposal 提交，经人工确认后生效并记录。 */
...
export function ModelingPage() {
  ...
  /** 科学检索→精读→验证（RAG→LLM→E2P 全链）；数值候选自动编译为证据篮 */
  ...
  return (
    <div>
      ...
      <div className="row" style={{ marginBottom: 16 }}>
        <button className="btn" onClick={compileEvidence} disabled={evidenceLoading}>
          {evidenceLoading ? (
            <>
        ...
        <button className="btn" onClick={runPolicy} disabled={modelPolicyLoading || loading || !gates?.modeling}>
          {modelPolicyLoading ? (
            <>
        ...
        <button
          ...
          onClick={runTraining}
          disabled={trainingLoading || loading || !gates?.modeling}
