/** ChatTab: Agent 对话（流式 + 幂等键 + 降级提示）。不占用主工作区，位于 Drawer 内。 */
          ...
          payload,
          {
            onEvent: (event) => {
              if (event.type === 'delta' && typeof event.content === 'string') {
                ...
                setStatus('error')
                setLastError(
                  String(event.summary ?? event.content ?? 'Agent 执行错误'),
                )
              }
            },
    ...
    <div className="chat-tab">
      ...
      {degraded && (
        <div className="warn-banner" style={{ margin: 8 }}>
          <span>
            {lastError ? lastError : 'Agent 当前不可用，已切换至标准科学计算模式。'}
          </span>
        </div>
      )}
      ...
      <div className="agent-messages">
        ...
        {messages.map((message) => (
          <div
            key={message.id}
            className={`agent-msg ${message.role}`}
